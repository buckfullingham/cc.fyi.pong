...
int Init(ESContext *esContext) {
  ...
  vertexShader =
      LoadShader(GL_VERTEX_SHADER, reinterpret_cast<const char *>(vShaderStr));
  fragmentShader = LoadShader(GL_FRAGMENT_SHADER,
                              reinterpret_cast<const char *>(fShaderStr));
  ...
  if (!linked) {
    ...
    if (infoLen > 1) {
      ...
      glGetProgramInfoLog(programObject, infoLen, NULL, infoLog.get());
